 ...
 * (admin checkbox on Categories). Product/checkout also require price ≥ N$5,000.
 */
export function categorySlugEligibleForPaymentPlan(categorySlug: string, categories: StoreCategoryDto[]): boolean {
  if (!categorySlug.trim() || categories.length === 0) return false
  const slugLower = categorySlug.trim().toLowerCase()
  const cat = categories.find((c) => c.slug.trim().toLowerCase() === slugLower)
  if (!cat) return false
