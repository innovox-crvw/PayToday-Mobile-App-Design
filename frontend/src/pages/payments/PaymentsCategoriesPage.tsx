...
function CategoryTile({ to, label, icon, caption }: { to: string; label: string; icon: ReactNode; caption?: string | null }) {
  return (
    <Box
...
export function PaymentsCategoriesPage() {
  ...
  const categoryTiles = hubTiles.fromDatabase ? hubTiles.items : PAYMENTS_HUB_TILES
  ...
  const sheet = (
    ...
    >
      ...
      <Grid container spacing={2} columns={4}>
        ...
        ) : hubTiles.fromDatabase && categoryTiles.length === 0 ? (
          <Grid size={{ xs: 4, sm: 4 }}>
            <Typography color="text.secondary" textAlign="center">
              No payment categories in the database yet. Add rows to{' '}
              <Typography component="span" variant="body2" sx={{ fontFamily: 'monospace' }}>
                hub_navigation_tiles
              </Typography>{' '}
              (hub_kind = &apos;payments&apos;) or check your API connection.
            </Typography>
          </Grid>
        ) : (
          categoryTiles.map((c) => (
            <Grid size={{ xs: 1, sm: 1 }} key={c.slug}>
              <CategoryTile
                to={resolveHref(pathPrefix, c.linkPath)}
                label={c.label}
                icon={hubNavIcon(c.iconKey)}
                caption={c.paymentMethodsCaption}
              />
            </Grid>
          ))
        )}
