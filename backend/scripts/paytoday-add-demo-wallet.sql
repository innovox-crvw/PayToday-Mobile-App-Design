/*
  Demo wallet: per-user balance + ledger for funding and hub / voucher wallet spends.
  Safe to run multiple times on an existing PayToday store database.
*/

IF COL_LENGTH('dbo.users', 'wallet_demo_balance_cents') IS NULL
BEGIN
